...
```bash
cd docker
cp ../.env .env        # add DOMAIN, POSTGRES_PASSWORD
docker compose up -d --build
docker compose exec app node -e "require('child_process').execSync('npm run db:push',{stdio:'inherit'})"
```

Compose runs the app, Postgres, Caddy for TLS, and a worker that calls
`POST /api/sync/run` every 15 minutes. The endpoint decides who is actually due,
so the timer only has to be more frequent than the shortest interval.
